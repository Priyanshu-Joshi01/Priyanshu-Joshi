1-- how many unique user
select count(distinct user_id) from users;

2-- how many cookie id for each user on avg
select count(cookie_id),user_id from clique_bait.users group by user_id;

3-- what is unique number of visits by all users per month
...
8-- what is number of views and cart adds for each product category 
